...
type Query[T any] interface {
	...
	FindOne(filter bson.M, opts *options.FindOneOptionsBuilder) (*T, error)
	FindAll(filter bson.M, opts *options.FindOptionsBuilder) ([]*T, error)
	FindPaginated(filter bson.M, page int64, limit int64, opts *options.FindOptionsBuilder) ([]*T, error)
	InsertOne(doc *T) (*bson.ObjectID, error)
	InsertAndRetrieveOne(doc *T) (*T, error)
	InsertMany(docs []*T) ([]bson.ObjectID, error)
	InsertAndRetrieveMany(docs []*T) ([]*T, error)
...
func (q *query[T]) FindOne(filter bson.M, opts *options.FindOneOptionsBuilder) (*T, error) {
	defer q.Close()
	var doc T
...
func (q *query[T]) FindAll(filter bson.M, opts *options.FindOptionsBuilder) ([]*T, error) {
	defer q.Close()
...
func (q *query[T]) FindPaginated(filter bson.M, page int64, limit int64, opts *options.FindOptionsBuilder) ([]*T, error) {
	defer q.Close()
	skip := (page - 1) * limit
...
func (q *query[T]) InsertOne(doc *T) (*bson.ObjectID, error) {
	defer q.Close()
...
func (q *query[T]) InsertMany(docs []*T) ([]bson.ObjectID, error) {
	defer q.Close()
	var iDocs []any
